...
class VipleBienvenueApp:
    ...
    def show_help_message(self):
        ...
        # Fermer automatiquement après 5 secondes et lancer le widget
        help_window.after(5000, lambda: self.close_and_launch_widget(help_window))
    
    def close_and_launch_widget(self, window):
        window.destroy()
        # Lancer le widget LaunchPad
        import subprocess
        import os
        script_dir = os.path.dirname(os.path.abspath(__file__))
        subprocess.Popen([
            'python', 
            os.path.join(script_dir, 'viple_launchpad.py')
        ])
